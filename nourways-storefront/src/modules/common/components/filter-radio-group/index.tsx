...
const FilterRadioGroup = ({
  ...
  return (
    <>
      <div className="md:flex gap-x-3 flex-col items-start gap-y-3 hidden">
          ...
          {items?.map((i) => (
            ...
            >
              ...
              <Label
                ...
                //   }
                // )}
                className="cursor-pointer text-base text-neutral-900"
      ...
      <div className="flex md:hidden">
        <select
          name="filter"
          id="filter-products"
          className=" text-neutral-500 text-base outline-none py-2 px-2 "
          onChange={(e) =>
            handleChange(
              e as unknown as ChangeEvent<HTMLButtonElement>,
              e.target.value
        ...
        >
          {items.map((i) => (
            <option key={i.value} id={i.value} value={i.value}>
              {i.label}
            </option>
          ))}
        </select>
